...
import { isMobile, isLowEnd, effectiveDPR, shouldEnableWebGL } from '../../js/mobile-detect';
    ...
    useEffect(() => {
        ...
        `;
            ...
            const navigateToSlide = (targetIndex: number) => {
                ...
                window.dispatchEvent(new CustomEvent('hero-slide-change', { detail: { index: currentSlideIndex } }));
            ...
            const initRenderer = async () => {
                ...
                if (!shouldEnableWebGL) {
                    // Show first slide as static background fallback
                    const canvas = document.querySelector(".webgl-canvas") as HTMLCanvasElement;
                    if (canvas) canvas.style.display = 'none';
                    const wrapper = document.querySelector('.slider-wrapper');
                    if (wrapper && slides[0]) {
                        (wrapper as HTMLElement).style.backgroundImage = `url(${slides[0].media})`;
                        (wrapper as HTMLElement).style.backgroundSize = 'cover';
                        (wrapper as HTMLElement).style.backgroundPosition = 'center';
                    }
                ...
                renderer.setSize(window.innerWidth, window.innerHeight); renderer.setPixelRatio(Math.min(effectiveDPR, 2));
